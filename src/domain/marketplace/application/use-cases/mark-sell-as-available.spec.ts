...
import { MarkSellAsAvailableUseCase } from './mark-sell-as-available'
...
let sut: MarkSellAsAvailableUseCase

describe('Mark Sell As Avaiable Use Case', () => {
  beforeEach(() => {
    ...
    sut = new MarkSellAsAvailableUseCase(
  ...
  it('should not allow marking a sold productt as available', async () => {
    const seller1 = makeSeller({}, new UniqueEntityId('seller-1'))
  ...
  it('should allow updating the product status to available', async () => {
    const seller = makeSeller({}, new UniqueEntityId('seller-1'))
